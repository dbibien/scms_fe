...
export const homeUpdateFormSchema = z.object({
  ...
  note: z.string().max(4500, { message: "Note must not exceed 4500 characters" }).optional(),
  ...
  house_check_note: z.string().max(4500, { message: "Note must not exceed 4500 characters" }).optional(),
  ...
  .refine((data) => {
    if (data?.house_check && !data.house_check_start_date) {
      return false
    }
    ...
    path: ["house_check_start_date"],
  })
  .refine(data => {
    if (data?.house_check && !data.house_check_end_date) {
      return false
    }
    return true
  },
...
export const createReportFormSchema = z.object({
  ...
  aiGeneartedNarative: z.string().min(1, "Missing content").optional(), //.max(500, "Exeeced 500 characters count"),
